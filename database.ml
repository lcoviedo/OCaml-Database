...
(* [make n] is the database with no contacts and at most [n] contacts
    stored inside. *)
let make max_number_of_contacts =
  {
    number_of_contacts = 0;
    contacts = Array.create max_number_of_contacts nobody
  };;
...
(* Queries are represented by a code and a contact.
   ...
   - If the code is 3 then the updates the number of an
     exsiting contact or inserts a new contact.  *)
type query = {
  code    : int;
  contact : contact;
}
  ...
  let rec swap a i =
    ...
    if not status then (false, db, contact)
      ...
      if db.contacts.(i).name = contact.name then
        let _ = db.contacts.(i) <- nobody in
        let _ = swap db.contacts i in
        db.contacts.(i)
      else
        db.contacts.(i) in
      let db' = {
...
let update db contact =
  let (status, db, _) = search db contact in
  if not status && db.number_of_contacts >= Array.length db.contacts then
    (false, db, nobody)
